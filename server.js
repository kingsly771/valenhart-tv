 ...
 *  - HLS stream proxy
 *  - JWT Auth (register / login / roles) — fully in-memory
 *  - Admin Panel API — fully in-memory
 *  - Per-user Favorites & History — in-memory
 */
...
// ── SQLite-backed store (falls back gracefully to in-memory) ──────────────
const { store: dbStore } = require('./db');
...
// ═══════════════════════════════════════════════════════
//  AUTH STORE — SQLite-backed via db.js
// ═══════════════════════════════════════════════════════
app.locals.authStore = dbStore;

// ═══════════════════════════════════════════════════════
//  IPTV IN-MEMORY STORE  (v3 compatible)
// ═══════════════════════════════════════════════════════
const store = {
  playlists:       [],
...
// Expose to admin route
app.locals.getActiveChannels = getActiveChannels;

// Playlist injection hook (called by admin route after adding via URL/content)
...
// ═══════════════════════════════════════════════════════
//  PLAYLIST ENDPOINTS  (v3 — unchanged)
// ═══════════════════════════════════════════════════════
...
// ═══════════════════════════════════════════════════════
//  FAVORITES  (v3 session-based — kept for anon users)
// ═══════════════════════════════════════════════════════
...
// ── Admin Panel — server-side JWT + admin-role guard ────────
// The client-side auth overlay in admin.html also enforces this,
// but we add a server-side check so direct URL access by non-admins
// gets redirected to home instead of showing the panel.
app.get('/admin-panel*', (req, res) => {
  ...
  if (token) {
    try {
      ...
      // Verified non-admin → redirect home
      if (user && user.role !== 'admin')
        return res.redirect('/?notice=admin_only');
    } catch { /* expired/bad token — let client overlay handle */ }
  }

  // No token or admin verified — serve page (client overlay gates login)
