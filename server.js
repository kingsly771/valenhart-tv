...
app.locals.injectPlaylist = ({ name, url, content, sqliteId }) => {
  const channels = parseM3U(content);
  if (!channels.length) return;
  const id = sqliteId || String(Date.now());
  store.playlists.push({ id, name, url, channels, loadedAt: new Date().toISOString() });
  store.activePlaylistId = id;
};

app.locals.removePlaylist = (sqliteId) => {
  store.playlists = store.playlists.filter(p => p.id !== sqliteId);
  if (store.activePlaylistId === sqliteId)
