...
app.post('/api/playlist/url', async (req, res) => {
  ...
  try {
    ...
    if (!response.ok) {
      if (response.status === 404) throw new Error('Playlist not found at this URL (404). Check the link is correct and not expired.');
      if (response.status === 403) throw new Error('Access denied (403). This playlist URL may require authentication or has expired.');
      if (response.status === 401) throw new Error('Authentication required (401). The playlist URL needs login credentials.');
      throw new Error(`Server returned HTTP ${response.status}. The URL may be invalid or temporarily unavailable.`);
    }
  ...
  try {
    const upstream = await fetch(decodedUrl, {
      headers: {
        ...
        'Accept':          '*/*',
        'Accept-Language': 'en-US,en;q=0.9',
        'Accept-Encoding': 'identity',           // avoid gzip on streams
    ...
    // Relay upstream status — don't silently serve 403/404 as 200
    if (!upstream.ok && upstream.status !== 206) {
      return res.status(upstream.status).json({ error: `Upstream returned ${upstream.status}` });
    }

    // Determine content-type: prefer upstream, but fix missing TS type
    let ct = upstream.headers.get('content-type') || '';
    const urlPath = decodedUrl.split('?')[0].toLowerCase();
    if (!ct || ct === 'application/octet-stream') {
      if (urlPath.endsWith('.m3u8') || urlPath.endsWith('.m3u')) ct = 'application/vnd.apple.mpegurl';
      else if (urlPath.endsWith('.ts'))  ct = 'video/MP2T';
      else if (urlPath.endsWith('.aac')) ct = 'audio/aac';
      else if (urlPath.endsWith('.mp4')) ct = 'video/mp4';
    }
    ...
    // Relay content-length if present (helps for TS segments)
    const cl = upstream.headers.get('content-length');
    if (cl) res.setHeader('Content-Length', cl);

    // For .m3u8 manifests: rewrite internal segment/playlist URLs through proxy
    const isManifest = ct.includes('mpegurl') || urlPath.endsWith('.m3u8') || urlPath.endsWith('.m3u');
    if (isManifest) {
      const text = await upstream.text();
      const base = getBaseUrl(decodedUrl);
      ...
      const rewritten = text.replace(/^(?!#)(.+)$/gm, (line) => {
        ...
        return `/api/proxy/stream?url=${encodeURIComponent(absUrl)}&referer=${encodeURIComponent(decodedUrl)}`;
      });
      res.setHeader('Content-Type', 'application/vnd.apple.mpegurl');
      return res.send(rewritten);
    }

    // Stream binary content (TS segments, MP4, etc.)
